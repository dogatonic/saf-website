...
/*========== Membership FORM INPUT VALIDATION (DON'T CHANGE) ==========*/
$(function () {
	// init the validator
	// validator files are included in the download package
	// otherwise download from http://1000hz.github.io/bootstrap-validator
		...
		if (!e.isDefaultPrevented()) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (messageAlert && messageText) {
						// inject the alert to .messages div in our form
						$("#membership-form").find(".messages").html(alertBox);
						// empty the form
...
$(document).ready(function () {
	$("#owl-carousel .carousel-wrap .owl-carousel").owlCarousel({
		...
		navText: [
			//font awesome prev & next buttons
			"<i class='fas fa-chevron-left'></i>",
			"<i class='fas fa-chevron-right'></i>",
		],
		responsive: {
			// set number of items shown at screen width
			0: {
...
$(document).ready(function () {
	$(window).scroll(function () {
		...
		if ($(this).scrollTop() > 500) {
			//if scrolled 500px from top of page
...
$(document).on("click", 'a[href^="#home"]', function (event) {
	//when "#home" (to header id) link is clicked
	event.preventDefault(); //cancel click event (to normal URL)

	$("html, body").animate(
		{
			//animate html/body (both for browser compatibility)
			scrollTop: $($.attr(this, "href")).offset().top, //scroll top to link "#home" location (header id)
		},
		1000
	); //top scroll speed (1000ms change to your preference)
});
...
$(document).ready(function () {
	//NOTE: ** only change query if you use a different filter nav id name **
	$("#navbarProjects .nav-link").on("click", function () {
		//when filter nav link is clicked
